...
//! `crate::mc`).
//!
//! Alcance (Fase 5 + Fase 6 "barreras bajo Q y exposicion", PLAN_PRODUCTS.md §12):
...
//! completa (`Add`/`Sub`/.../`Clamp`/`EventValue`) y predicados de comparacion/`Between`/`All`/
//! `Any`/`Not`/`EventOccurred`. La resolucion de `Trigger` replica en Rust
...
pub use api::{
    hit_probability_gbm_q, payoff_exposure_profile_gbm_q, payoff_sensitivity_gbm_q, price_payoff_exercise_gbm_q,
    price_payoff_gbm_q, ExercisePolicyResult,
};
