//! Snapshot de mercado: la pieza que faltaba entre "un modelo con parámetros elegidos a mano"
//! (PLAN.md §5.2, `HullWhite1F` con `a`/`b`/`sigma` constantes) y "un modelo calibrado a algo
//! observable" (`crate::calibration`). Un `MarketSnapshot` es solo datos — `pillars`/
//! `zero_rates` en `f64` puro, sin tipos de Burn — igual que `Params` no es una jerarquía
//! polimórfica: si el mercado es "falso" (fabricado para un test o una demo,
//! `synthetic_from_hull_white`) o "real" (tipos observados y bootstrapeados fuera de este
//! crate) es una cuestión de *de dónde salen los números*, no de un tipo distinto.
//!
//! **Simplificación deliberada de esta primera versión** (igual espíritu que la de
//! `crate::models::hull_white`): el snapshot ya viene como una curva cero-cupón (`pillars` +
//! tipos cero continuos), no como los instrumentos de mercado crudos (depósitos, futuros,
//! swaps) de los que se bootstrapea una curva real — ese bootstrapping es trabajo futuro, no
//! bloquea la calibración de un modelo a una curva ya construida.

/// Curva de mercado observada (o fabricada) en un instante dado: pares `(pillar, zero_rate)`
...
impl MarketSnapshot {
    /// Construye un snapshot a partir de pillars/zero_rates ya observados (mercado "real") o
    /// fabricados a mano (mercado "falso"): esta función no distingue entre ambos casos, ver
    ...
    pub fn new(pillars: Vec<f64>, zero_rates: Vec<f64>) -> Self {
        ...
        assert!(!pillars.is_empty(), "MarketSnapshot necesita al menos un pillar");
    ...
    /// Mercado "falso" (PLAN.md): fabrica un `MarketSnapshot` leyendo la propia fórmula
    /// cerrada de `HullWhite1F` (`crate::smoke::hull_white_zero_coupon_bond`) en los pillars
    /// dados. Útil para probar/demostrar `crate::calibration` sin depender de datos de
    /// mercado reales: generar con unos parámetros conocidos, calibrar desde otra estimación
...
mod tests {
    ...
    fn zero_rate_interpolates_linearly_between_pillars() {
        let market = MarketSnapshot::new(vec![1.0, 2.0, 5.0], vec![0.02, 0.03, 0.04]);
        assert!((market.zero_rate(1.0) - 0.02).abs() < 1e-12);
        assert!((market.zero_rate(2.0) - 0.03).abs() < 1e-12);
        // A mitad de camino entre 2.0 (0.03) y 5.0 (0.04): interpolación lineal simple.
        assert!((market.zero_rate(3.5) - 0.035).abs() < 1e-12);
    }

    #[test]
    fn zero_rate_extrapolates_flat_outside_pillars() {
        let market = MarketSnapshot::new(vec![1.0, 5.0], vec![0.02, 0.04]);
        assert!((market.zero_rate(0.1) - 0.02).abs() < 1e-12);
        assert!((market.zero_rate(10.0) - 0.04).abs() < 1e-12);
    }

    #[test]
    fn discount_factor_matches_continuous_compounding_formula() {
        let market = MarketSnapshot::new(vec![1.0, 2.0], vec![0.02, 0.02]);
        let expected = (-0.02_f64 * 2.0).exp();
        assert!((market.discount_factor(2.0) - expected).abs() < 1e-12);
    }
    ...
    fn synthetic_from_hull_white_reproduces_the_models_own_prices() {
        ...
        let market = MarketSnapshot::synthetic_from_hull_white(a, b, sigma, r0, pillars.clone());
