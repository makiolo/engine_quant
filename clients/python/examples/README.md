...
- `price_flow.py` — flujo completo de `ENGINE.PRICE` (PLAN.md §7.15): `q.IRSwap`/
  `q.HullWhite1F`/`q.Market` construidos como objetos tipados, `q.Engine(...)` fija
  `PricingContext`/`ExecutionContext` en el constructor, luego `q.Engine.price(...)` calcula un
  lote de medidas (`PV`, `DV01`, `ExpectedExposure`, `PFE95`, `UnilateralCVA`) de una sola vez.
  Incluye también `q.IRSwap.par(...)` (swap "a la par", PLAN_REAPI.md §3.2).
...
- `price_batch_flow.py` — los tres niveles de la API de cálculo por lotes (PLAN.md §7.19) sobre
  `q.Engine`: `price_batch` (lote homogéneo, mismo calendario), `price_many` (lote heterogéneo,
...
- `greeks_flow.py` — flujo completo de PLAN_GREEKS.md §9.1: autoría programática del payoff →
  `q.Engine.price(...)` → `q.Engine.all_greeks(...)`, sin código nuevo por factor de riesgo.
