...
`price_flow.py` no cubre: `qd.DV01(bump=...)`/`qd.DV01(bucketed=True)` -- medidas con
...
def main():
    ...
    # en la misma llamada -- `quantdesk.Engine.price` traduce `qd.PV()`/`qd.DV01()` a su spec
    # internamente, igual que hacía `eng.price(..., [qd.PV().to_spec(), ...])` antes.
    measures = [qd.PV(), qd.DV01(), "ExpectedExposure", "PFE95", qd.UnilateralCVA()]
    result = qeng.price(trade, model, market, measures)

    print(f"PV             = {result.PV.scalar:,.2f}")
    print(f"DV01 (1bp)     = {result.DV01.scalar:,.2f}")
    dv01_2bp = qeng.price(trade, model, market, [qd.DV01(bump=0.0002)])
    ...
    bucketed = qeng.price(trade, model, market, [qd.DV01(bucketed=True)]).DV01
