...
Curve::Curve(std::vector<double> pillars, std::vector<double> zero_rates)
    : pillars_(std::move(pillars)), zero_rates_(std::move(zero_rates)) {
    if (pillars_.size() != zero_rates_.size()) {
        throw std::invalid_argument("Curve: pillars y zero_rates deben tener el mismo tamano");
    }
    if (pillars_.empty()) {
        throw std::invalid_argument("Curve: necesita al menos un pillar");
    }
    for (std::size_t i = 1; i < pillars_.size(); ++i) {
        if (pillars_[i] <= pillars_[i - 1]) {
            throw std::invalid_argument("Curve: pillars debe ser estrictamente creciente");
        }
    }
}
...
double Curve::discount_factor(double t) const {
    return std::exp(-zero_rate(t) * t);
}

Curve Curve::synthetic_from_hull_white(
    ...
    return Curve(pillars, std::move(zero_rates));
}

Curve Curve::synthetic_from_hull_white_2f(
...
MarketSnapshot::MarketSnapshot(
    ...
    : discount_curve_(std::move(pillars), std::move(zero_rates)),
